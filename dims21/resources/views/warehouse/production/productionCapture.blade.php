                        ...
                        <div class="row">
                            ...
                            <div class="form-group mb-2 col-6">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <input type="checkbox" value="0" id="checkShift">
                                </div>
                            ...
                            <div class="form-group mb-2 col-6">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <input type="checkbox" value="0" id="checkDept">
                                </div>
                            ...
                            <div class="form-group mb-2 col-6">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <input type="checkbox" value="0" id="checkMach">
                                </div>
                            ...
                            <div class="form-group mb-2 col-6">
                                ...
                                <input type="number" step="any" class="form-control w-100" id="inputSageWeight"
                                    disabled>
                            </div>

                            <div class="form-group mb-2 col-6">
                                <label class="control-label fw-bold" for="inputActualWeight">Actual Weight</label>
                                <input type="number" step="any" class="form-control w-100" id="inputActualWeight"
                                    required>
                            </div>
                            ...
                            <div class="form-group mb-2 col-3">
                                ...
                                <div id="inputDowntime1" class="d-inline-flex w-100">
                                    <input type="number" min="0" step="1"
                                        class="form-control me-2 hours" />
                                    <h3>:</h3>
                                    <input type="number" min="0" max="59" step="1"
                                        class="form-control ms-2 minutes" />
                                </div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            var departments = $.map(JSON.parse(JSON.stringify({!! json_encode($departments) !!})), function(item) {
                return {
                    id: item.intAutoID,
                    text: item.strDeptName
                }
            });
            ...
            $('#selectDepartment').empty().select2({
                data: departments,
                theme: 'bootstrap-5',
                dropdownParent: $('#captureModal'),
            });
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (typeof(Storage) !== "undefined") {
                                var machcheck = localStorage.getItem("machcheck");
                                var machval = localStorage.getItem("machval");

                                if (machcheck && machcheck === "1") {
                                    $("#selectMachine").val(machval).trigger('change');
                                    $("#checkMach").prop("checked", true);
                                }
                            }
                        }
                    });
            ...
            function getStorageData(){
                if (typeof(Storage) !== "undefined") {
                    ...
                    var shiftcheck = localStorage.getItem("shiftcheck");
                    var deptcheck = localStorage.getItem("deptcheck");

                    var dateval = localStorage.getItem("dateval");
                    var shiftval = localStorage.getItem("shiftval");
                    var deptval = localStorage.getItem("deptval");

                    if (datecheck && datecheck === "1") {
                        $("#inputDate").val(dateval);
                        $("#checkDate").prop("checked", true);
                    }
                    if (shiftcheck && shiftcheck === "1") {
                        $("#selectShift").val(shiftval);
                        $("#checkShift").prop("checked", true);
                    }
                    if (deptcheck && deptcheck === "1") {
                        $("#selectDepartment").val(deptval).trigger('change');
                        $("#checkDept").prop("checked", true);
                    }
                } else {
                    alert("Sorry, your browser does not support local storage and you wont be able to store date, shift, department, and machine");
                }
            };

            function setStorageData(){
                var datecheck = $("#checkDate").prop("checked") ? "1" : "0";
                var shiftcheck = $("#checkShift").prop("checked") ? "1" : "0";
                var deptcheck = $("#checkDept").prop("checked") ? "1" : "0";
                var machcheck = $("#checkMach").prop("checked") ? "1" : "0";

                var dateval = $("#inputDate").val();
                var shiftval = $("#selectShift").val();
                var deptval = $("#selectDepartment").val();
                var machval = $("#selectMachine").val();

                localStorage.setItem("datecheck", datecheck);
                localStorage.setItem("shiftcheck", shiftcheck);
                localStorage.setItem("deptcheck", deptcheck);
                localStorage.setItem("machcheck", machcheck);

                localStorage.setItem("dateval", dateval);
                localStorage.setItem("shiftval", shiftval);
                localStorage.setItem("deptval", deptval);
                localStorage.setItem("machval", machval);
            };
        });
    </script>
