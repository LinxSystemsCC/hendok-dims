...
<div class="modal-body">
    <form action="{{ route('system-modules.update', $moduleData->intAutoId) }}" class="update-system-module-form">
        @csrf
        <div id="general-error"></div>
    
        <div class="form-group" id="addCustomerDiv">
            <label class="control-label" for="moduleName" style="margin-bottom: 0px; font-weight: 700; font-size: 15px;">Name</label>
            <input type="text" class="form-control input-sm col-xs-1" id="moduleName" name="moduleName" value="{{ $moduleData->strName }}">
        </div>
            ...
            <select class="form-select dims-select2" id="intParentId" name="intParentId" required>
                ...
                @foreach ($nameList as $val)
                    <option value="{{ $val->intAutoId }}" 
                        {{ $val->intAutoId == $moduleData->intParentsId ? 'selected' : '' }}>
                        {{ $val->strName }}
                    </option>
                @endforeach
            </select>
