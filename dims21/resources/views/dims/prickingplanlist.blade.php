                ...
                <table style="font-size: 11px;">
                    <tbody>
                        <tr>
                            ...
                            <td class="col-xs-2" style="width: 117px;">
                                @foreach ($pickingheader as $value)
                                    {{ $value->intDunnages }}
                                @endforeach
                            </td>
                        ...
                        <tr>
                            ...
                            <td class="col-xs-2">
                                @foreach ($pickingheader as $value)
                                    {{ $value->strCheckerName }}
                                @endforeach
                            </td>
                            <td class="col-xs-2">Plastic Corners #</td>
                            <td class="col-xs-2">
                                @foreach ($pickingheader as $value)
                                    {{ $value->intPlasticCorners }}
                                @endforeach
                            </td>
                            <td class="col-xs-2"></td>
                            <td class="col-xs-2"></td>
...
'left=20,top=20,width=1000,height=1000,toolbar=1,resizable=0'); return false;"
        ...
        @foreach ($pickingheader as $value)
            @if ($value->isCancelled == '0')
                <button class="planstatus" value="1" style="background: #bb1523;float: right;">Delete This
                    Plan</button>
            @endif

            @if ($value->isCancelled == '1')
                <button class="planstatus" value="0" style="background: #0BA008;float: right;">Re-Activate This
                    Plan</button>
            @endif
        @endforeach
        <br>
    ...
    $(document).ready(function () {
        ...
        $('#orderHeaderPrint').on('click', '.invoicethis, .ibt', function (e) {
            ...
            if (strTicket == "notick") {
                ...
                if (ubARIBT == 1) {
                    window.open('{!! url('/whtransfers') !!}/' + $('#refno').val() + "/" + ibt,
                        "whtransfers" + $('#refno').val(),
                        "location=1,status=1,scrollbars=1, width=1200,height=850");
                } else {
                    $.ajax({
