...
<body style="font-family: Sans-serif">
    ...
    <a href='{!! url('/viewpickingtickets') !!}'
        onclick="window.open(this.href, 'viewpickingtickets',
'left=20,top=20,width=1000,height=1000,toolbar=1,resizable=0'); return false;"
        style="background: red;padding:10px;color: black;font-weight: 900">My Picking Sips</a>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#print').click(function() {
            window.open('{!! url('/pickingplanlist') !!}/' + $('#ref').val(), "plan" + $('#ref').val(),
        ...
        $('#save').click(function() {
            ...
            if (selectedRo.length < 1) {
                var dialog = $('<p><strong style="color:black"> Please Assign Route </strong></p>')
                    .dialog({
                        height: 200,
                        width: 900,
                        modal: true,
                ...
                var linesWithPlannedQty = allGridItems.filter(element =>
                    element.items.some(item => item.qtyAlreadyPlanned > 0)
                );

                if (linesWithPlannedQty.length > 0) {
                    var confirmationMessage = "These lines have already been planned:\n";
                    linesWithPlannedQty.forEach(element => {
                        element.items.forEach(item => {
                            if (item.qtyAlreadyPlanned > 0) {
                                confirmationMessage +=
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        window.open('{!! url('/plannickname') !!}/' + $('#ref').val(),
                            "plannickname" + $('#ref').val(),
                            "location=1,status=1,scrollbars=1, width=1200,height=850");
            ...
            success: function(data) {
                ...
                $("#gridContainer").dxDataGrid({
                    ...
                    onContentReady: function(e) {
                        ...
                        console.log("work wen 2");
                        selectedDatasUsers.forEach((element, index, array) => {
                            console.log(element.items[0]); // 100, 200, 300
                            qty = qty + (parseFloat(element.items[0].Mass) * parseFloat(
                                element.items[0].qtyPlan));
                    ...
                    onRowPrepared(e) {
                        ...
                        if (e.rowType == 'data' && e.data.isRoofing != "Roofing" && parseInt(e.data
                                .qtyPlan) > parseInt(e.data.QtyOnHand)) {
